	...
	public function display_config(Request $request)
	{
		if ($request->req == 'get-data') {
			...
			if ($call) {
				...
				return response()->json(['voice' => "Nomor Antrian '$kd, $no' Silahkan Menuju ke Meja ".$call->loket_id, 'no_antrian' => $call->no_antrian, 'loket' => $call->loket_id, 'next' => $next], 200);
			} else {
				return response()->json(['voice' => null, 'no_antrian' => '0-000', 'loket' => 1, 'next' => null], 200);
			}
